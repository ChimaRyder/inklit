<div>
    <h1 class="text-4xl font-bold mb-3 text-center">{{ $heading }}</h1>
    <form class="flex flex-col gap-4" wire:submit="{{ $submitFunction }}" method="POST">
        @csrf
        ...
        <button class="btn btn-lg bg-amber-700 text-white" @disabled(!$canSubmit) type="submit">{{ $submitButton }}</button>
    </form>
</div>
